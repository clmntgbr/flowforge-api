...
type StepRun struct {
	...
	Status   enum.StepRunStatus   `gorm:"type:varchar(20);not null;check:status IN ('pending','running','completed','failed','canceled');index:idx_step_run_status;index:idx_step_run_workflow_status,priority:2" json:"status"`
